...
import {
  WorkshopEnrolledStudent,
  WorkshopEnrolledStudentSummary,
...
@Component({
  selector: 'app-enrolled-workshop-students',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './enrolled-workshop-students.html',
  styleUrl: './enrolled-workshop-students.scss',
})
export class EnrolledWorkshopStudents implements OnInit {
  ...
  summary: WorkshopEnrolledStudentSummary = this.createDefaultSummary();
  ...
  async loadStudents(page = 1): Promise<void> {
    ...
    try {
      const response = await lastValueFrom(
        this.workshopService.getEnrolledStudents(this.buildPayload(page)).pipe(timeout(15000)),
      );
  ...
  trackByStudent(_: number, student: WorkshopEnrolledStudent): number {
    return student.id;
  }
  ...
  formatDateRange(student: WorkshopEnrolledStudent): string {
    const startDate = student.programStartDate || '';
    const endDate = student.programEndDate || '';
  ...
  formatTimeRange(student: WorkshopEnrolledStudent): string {
  ...
  getProgramLocationLabel(student: WorkshopEnrolledStudent): string {
    return (
  ...
  getProgramAddress(student: WorkshopEnrolledStudent): string {
    return `${student.programBranchAddress || ''}`.trim();
  }

  getPaginationLabel(): string {
    return `Showing ${this.meta.from || 0}-${this.meta.to || 0} of ${this.meta.total} workshop enrollments`;
  }
  ...
  private createDefaultSummary(): WorkshopEnrolledStudentSummary {
